---
title: "When the Official Way Fails: REST API, Anti-Bot AI, and Why I Spent 5 Hours Building a Bridge Nobody Should Need"
date: 2025/11/07
...
tag: wordpress, devops, ci-cd, automation, rest-api, ssh, wpcli, siteground, cms, ai-agents, dfw-style
author: Aug
---
...
Here's what I thought would happen: write some Markdown files[^1], push to GitHub, let Actions convert them to HTML, POST them to WordPress via the REST API[^2], done. Thirty minutes, tops. Maybe an hour if the API documentation is particularly WordPress-esque[^3].

[^1]: Because in 2025 we're still pretending that everyone wants to write content in a WYSIWYG editor that mangles your HTML and adds seventeen `<span>` tags with inline styles nobody asked for. Markdown is what humans write. WordPress is what marketers think humans want.
...
[^3]: WordPress documentation exists in this quantum state where it's simultaneously comprehensive and completely useless. You can find detailed docs about every filter hook since 2.7, but good luck finding a straightforward answer to "how do I publish a page via API without getting a 401."

Here's what actually happened: I spent 4+ hours fighting SiteGround's Anti-Bot AI[^4], rewrote the entire workflow to use SSH + WP-CLI, debugged six separate SSH implementation issues[^5], and ended up with a working system that bypasses HTTP entirely because—and I cannot stress this enough—*WordPress is fundamentally hostile to automation*.

[^4]: More on this later, but preview: SiteGround's "Anti-Bot AI" is like TSA security theater except instead of confiscating your shampoo, it blocks every legitimate API request from GitHub Actions while presumably letting actual bots through because they're smart enough to use residential proxies.

[^5]: Including the delightful discovery that GitHub Actions runners are ephemeral, which means your SSH agent dies between steps, which means you have to restart it and re-add your key with an expect script *in every single step*, which is the kind of developer experience that makes you question your career choices.

This is that story. It's about WordPress, yes, but really it's about how we've built these baroque, self-important content management systems that are *technically* open-source and *theoretically* API-first but *practically* unusable for the exact use case they claim to support: automated content publishing[^6].

[^6]: And by extension: AI agents. If GitHub Actions—which is literally just running Python scripts with curl—can't reliably talk to WordPress, how exactly are we expecting AI agents to do it? Spoiler: we're not. WordPress is for humans clicking buttons in wp-admin. Everything else is cosplay.
...
The request never reaches WordPress. My Application Password never gets checked. WordPress never sees the request at all. It's blocked at the infrastructure layer—probably nginx or a WAF module—and redirected to a captcha page[^11].

[^11]: Which, by the way, returns HTTP 202 instead of 403 or 401, because apparently the security theater extends to status codes. "We're not blocking you! We're just... asking you to prove you're human first. Indefinitely. With no way to bypass it programmatically."
...
SiteGround's Site Tools has a "Whitelist IPs" feature. Great! I'll just add GitHub's IP ranges—oh wait, GitHub Actions uses Azure's infrastructure, which is approximately *640,000 IP addresses*[^12]. And SiteGround's whitelist field has a character limit.
...
I draft this ticket. Stare at it. Realize that even if they respond promptly (they won't), even if they're helpful (maybe?), even if they whitelist the IPs (GitHub will change them eventually)... this is not a sustainable solution[^13].

[^13]: This is the moment where you realize you're fighting the system, not working with it. WordPress says "yes, use our REST API!" SiteGround says "yes, we support WordPress!" But together? Together they say "lol, no, you're not automating anything today."
...
SiteGround's Anti-Bot AI protects HTTP/HTTPS endpoints. But I have SSH access to the server[^14]. And WordPress has WP-CLI[^15]. And WP-CLI can do everything the REST API can do, except it runs *on the server*, so there's no HTTP request to block.

[^14]: SiteGround gives you SSH access even on shared hosting plans, which is actually pretty great. It's not a full shell—you're jailed to your home directory—but it's enough to run commands, including WP-CLI.

[^15]: WP-CLI is WordPress's command-line interface. It's been around since 2013, it's officially supported, and it's *powerful*. `wp post create`, `wp post update`, `wp cache flush`, `wp db query`—it can do basically anything. The WordPress REST API is really just a HTTP wrapper around functionality that WP-CLI has had for years.
...
GitHub Secrets stores your SSH key as a string. When you echo it to a file, newlines get mangled[^16]. The key looks fine in the logs (which are redacted anyway), but OpenSSH rejects it.

Fix: `echo "${{ secrets.SSH_PRIVATE_KEY }}" | tr -d '\r' > ~/.ssh/id_ed25519`

[^16]: This is a classic "it works on my machine" problem. Locally, your SSH key is a file with proper newlines. In GitHub Secrets, it's a string. When you echo a multi-line string in bash, things get weird. You can work around it with `tr`, or by using the `$''` syntax, or by just accepting that secrets management is forever cursed.

**Challenge 2: Expect Script Paths** (20:45)  
The expect script needs the *full path* to the SSH key. `~/.ssh/id_ed25519` doesn't expand in the expect context because expect is its own interpreter[^17].

Fix: Use `$HOME/.ssh/id_ed25519` or `/home/runner/.ssh/id_ed25519`

[^17]: Expect is a weird language. It's Tcl-based (remember Tcl?), it's from 1990, and it's *still* the best tool for automating interactive prompts. There are newer alternatives—pexpect for Python, expect-lite for bash—but plain old expect is what's installed everywhere. So you learn its quirks, like how it doesn't expand `~` and needs explicit paths.
...
YAML sees `<<` and gets confused[^18]. Is it a YAML thing? A bash thing? Both? The error message is useless.
...
[^18]: YAML has its own heredoc-like syntax with `|` and `>` and various modifiers (`|-`, `|+`, `>-`, etc.) that control newline handling. Bash *also* has heredocs with `<<`. When you nest bash heredocs inside YAML's pipe-multiline syntax, you've entered a circle of hell Dante didn't know about. The YAML parser and bash's parser both try to interpret the `<<`, and what you get is a parse error that mentions line numbers that don't correspond to anything in your file.

**Challenge 4: RSA vs ED25519** (20:54)  
My workflow was looking for `~/.ssh/id_rsa`. My actual key is ED25519[^19]. OpenSSH couldn't find the key.

Fix: Change all `id_rsa` references to `id_ed25519`

[^19]: ED25519 is the modern SSH key format. It's faster, more secure, and generates shorter keys than RSA. If you're still using RSA keys, you should switch. But lots of documentation (including older GitHub docs) assumes RSA, so you'll see `id_rsa` everywhere even though `id_ed25519` is better.
...
Three layers of escaping: YAML → Bash → SSH → WP-CLI[^21].
...
1. **The REST API is theater**  
   It exists. It's documented. It *technically* works. But managed hosts block it, rate limit it, or wrap it in so much security that you can't use it for automation[^22].

[^22]: To be clear: I'm not saying the REST API is *bad*. For building JavaScript frontends—headless WordPress, decoupled architectures—it's great. But for CI/CD? For AI agents? It's unusable. And nobody at WordPress or in the hosting ecosystem seems to think this is a problem worth solving.
...
What they're *not* optimizing for: developers who want to use WordPress programmatically[^23].

[^23]: Which is fine! That's not their target market. SiteGround's ideal customer is a small business owner who logs into wp-admin once a week to add a blog post. They're not trying to serve developers building CI/CD pipelines. The problem is WordPress *claims* to support that use case with the REST API, and hosts *claim* to support WordPress, but nobody's actually testing whether they work together.
...
4. **No cost-effective testing**: Can't spin up WordPress in a container to test; need real hosting[^24]

[^24]: Well, you *can* run WordPress in Docker locally. But the production environment—with all its security plugins, caching layers, and hosting-specific quirks—is impossible to replicate. So you test locally, it works, you deploy, it breaks in new and creative ways.
...
Unless you're willing to SSH into the server and use WP-CLI, which is exactly what I ended up doing[^27].

[^27]: And which I've now open-sourced [LINK TK] so you don't have to spend 5 hours debugging SSH key newlines and expect scripts. You're welcome.
...
No HTTP requests. No REST API. Just SSH + WP-CLI[^28].

[^28]: The full workflow is ~277 lines of YAML, plus documentation. It handles:
...
The code is open source: [GITHUB_LINK_HERE]

### Why It Feels Wrong

Because *this shouldn't be necessary*.

The REST API should work. Hosting providers should whitelist legitimate automation. WordPress should provide better tooling for CI/CD[^29].
...
Then this bridge is for you[^31].

[^31]: Clone the repo, add your SSH credentials to GitHub Secrets, update the hostname and paths, push a Markdown file. It should work. If it doesn't, open an issue. I've already debugged all the SSH key newline edge cases so you don't have to.
...
Then *don't use WordPress*[^32]. Use Notion, use Airtable, use Contentful, use any headless CMS built after 2015. They have APIs that actually work.

[^32]: I know, I know—"but WordPress has plugins!" Yes. 60,000 plugins. Most of them abandoned, some of them malicious, all of them adding their own database tables and filters and hooks that slow down your site and introduce security holes. The plugin ecosystem is WordPress's strength and its curse. For AI agents and automation, you want *less* surface area, not more.
...
2. **Security theater breaks legitimate use cases**  
   SiteGround's Anti-Bot AI blocks GitHub Actions (Microsoft/Azure IPs, legitimate API calls) while actual bots use residential proxies and get through fine[^33].

[^33]: This is the fundamental problem with IP-based bot detection: sophisticated bots use proxies, legitimate automation uses data center IPs. So you end up blocking the good guys and letting the bad guys through. But IP-based detection is cheap and easy, so hosting companies do it anyway.
...
4. **The AI agent future requires better CMSs**  
   If GitHub Actions can't reliably publish to WordPress, how will AI agents? We need CMSs designed for programmatic access from the ground up[^34].

[^34]: This is what Notion, Contentful, Strapi, and other modern headless CMSs get right: API-first design. The API isn't bolted on, it's the primary interface. The UI is a client of the API. This means the API actually works, because that's how the company's own tools use it.
...
The REST API is elegant in theory. SSH + WP-CLI is ugly in practice. But ugly-and-working beats elegant-and-broken[^35].

[^35]: This is a lesson I keep relearning. Beautiful abstractions that don't work are worse than ugly hacks that do. Python's `subprocess` is a terrible API—seriously, go read the docs, it's a mess—but it *works*. JavaScript Promises are an elegant abstraction, but `async`/`await` is the ugly syntax that makes them usable. WordPress REST API is clean and RESTful and blocked by hosting security; SSH + WP-CLI is old-school and imperative and *gets the job done*.

---

## Appendix: The 29 Commits

For those who want the full journey, here's the [GitHub repository](GITHUB_LINK_HERE) with all 29 commits. You can watch in real-time as I:

- Start with naive optimism (commit 1)
- Hit the SiteGround wall (commits 6-9)  
- Pivot to SSH (commit 10)
- Debug six SSH issues (commits 11-16)  
- Achieve success (commit 17)
- Write 1,000+ lines of documentation (commits 18-29)

It's a case study in technical problem-solving, persistence, and the gap between how things *should* work and how they *actually* work.

The code is MIT licensed. Use it. Fork it. Improve it. Or better yet: build a CMS that doesn't need these kinds of bridges in the first place[^36].

[^36]: And if you do, let me know. I'll write a blog post about it. Without footnotes. Promise.

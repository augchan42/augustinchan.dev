...
The bot had been using `agent-twitter-client`, which is one of those unofficial libraries that does the thing Twitter doesn't want you to do, which is to say it logs in like a browser and scrapes the DOM[^2]. This worked fine until it didn't—cookies would expire, Twitter would change their HTML structure, the rate limiting was mysterious and seemed almost personal—and more importantly it violated the Terms of Service, which matters if you're trying to run something that won't get banned.
...
Also discovered during this documentation spree: The free tier has a 10,000 tweet read cap *per month*. Not rate limits per 15 minutes—which would be annoying but manageable—but a hard monthly cap, like a cell phone plan from 2005.

This will become relevant approximately 16 hours later.
...
Here's where the math got ugly: Every interaction check cost 20 tweets from the monthly cap. The bot was polling every 120 seconds. That's 720 checks per day, which is 14,400 tweets per month.

You see the problem.

The bot exceeded the free tier limit in 16 hours, which is faster than most people finish a Netflix series.
...
New math: 20 tweets per check × 288 checks per day = 5,760 tweets per month.

Under 10,000. Sustainable. Problem solved.
...
Final architecture:
- v2 API for all reads (10,000/month cap)
...
- ✅ Reading up to 10,000 tweets/month (v2)
- ✅ Mention monitoring (if optimized with `since_id`)

**What doesn't:**
- ❌ v1.1 search/timeline fallbacks (blocked)
- ❌ More than 10K reads/month
- ❌ Real-time timeline streaming
- ❌ My original plan

**Optimization results:**
- Before: 14,400 tweets/month → exhausted in 16 hours
- After: 5,760 tweets/month → sustainable within 10K limit
...
After 10 commits over three days, I built a system that works within Twitter's free tier by accepting that I can't work around it. Which sounds like defeat but is actually just engineering: You work with the constraints you have, not the constraints you wish you had.

The bot posts. The bot reads mentions. The bot stays under 10,000 tweets per month. It's not elegant, but it works, and working is the thing that matters when you've spent three days optimizing API usage instead of building actual features.
...
[^2]: Document Object Model, the browser's representation of a webpage's structure, which you're supposed to interact with via official APIs rather than pretending to be a browser and parsing HTML like some kind of digital burglar.
